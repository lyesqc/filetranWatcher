...
public class JsonFileParser extends FileParser {
	...
	private String extractAttributeValues(JsonNode node, String attribute) {
		{
			...
			/**
			 * we have two case : 1.node contain an array of value, of for [val1,val2,val3]
			...
			if (node.get(attribute) != null && node.get(attribute).isArray()) {
				...
				while (valueArray.hasNext()) {
					...
					if (LOGGER.isDebugEnabled()) {
						LOGGER.debug(valueNode.toString());
					}
	...
	/**
	 * it is recursive method,get all object of tree, stop on last object node each
	 * returned object will contain a set of requested attributes
	 * 
	 * @param node      : node to check if it is researched node or not, if yes add
	 *                  it to list, else call method to its child
	...
	public void getListNodeObject(JsonNode node, String[] delimitor, List<JsonNode> listNode) {
		...
		/**
		 * of current delimiter not equal to current node then flush listNode and exit,
		 * meaning delivered access path not convenient to file format
		 */
